...
\begin{document}
	...
	Целью проведения практики является формирование у обучающегося навыков поиска, анализа и структурирования научных работ для написания аналитических обзоров.
	Практический опыт позволит четко выявить проблемы, тенденции и перспективные направления для выбранной темы выпускной квалификационной работы.
	...
	После финальной проверки списка источников, было выявлено, что большинство источников старше 5 лет, что нарушает требование работы.
	Исходя из этого, некоторые источники были заменены смежными современными публикациями:
	\begin{itemize}
		\item обзор по алгоритмам кластеризации \repsrcref{40} был заменен на \repsrcref{24};
		\item обзор по детектированию сообществ \repsrcref{41} был заменен на \repsrcref{25};
		\item публикация про Leiden и сравнение вариантов кластеризации \repsrcref{42} была заменена на \repsrcref{26}, где обсуждаются решения, связанные с Leiden, и сравниваются альтернативы;
		\item методологическая работа по PMI/NPMI \repsrcref{43} была заменена на статью про интерпретацию PMI \repsrcref{27};
		\item источник про валидацию кластеризации (силуэт и близкие индексы) \repsrcref{44} был заменен на \repsrcref{28};
		\item источник про эмбеддинги \repsrcref{45} был заменен на \repsrcref{29}.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item Публикация, рассматривающая варианты построения кластеризационных решений на графах и сравнивающая альтернативы (в том числе Leiden и близкие подходы), что полезно при обосновании выбора алгоритма для задач масштабной кластеризации \repsrcref{26}.

		\item Методологическая работа, обсуждающая pointwise mutual information (PMI) и его интерпретацию при анализе совстречаемости (включая нормированные варианты вроде NPMI) \repsrcref{27}.

		\item Обзор индексов валидации кластеризации, включающий метрику силуэта (silhouette) и практику её применения \repsrcref{28}.

		\item Обзор по эмбеддингам, необходимый для описания векторных представлений запросов и терминов \repsrcref{29}.
	\end{itemize}
	...
	\begin{enumerate}[wide=0pt, leftmargin=0pt, labelindent=\parindent, listparindent=\parindent, nosep, label=\arabic*]
		...
		\item \hypertarget{rep:i24}{} Ezugwu A. E., Ikotun A. M., Oyelade O. O., Abualigah L., Agushaka J. O., Eke C. I., Akinyelu A. A. A comprehensive survey of clustering algorithms: State-of-the-art machine learning applications, taxonomy, challenges, and future research prospects. // Engineering Applications of Artificial Intelligence. -- 2022. -- Т. 110. -- Article 104743. DOI: 10.1016/j.engappai.2022.104743.
		\item \hypertarget{rep:i25}{} Chunaev P. Community detection in node-attributed social networks: A survey. // Computer Science Review. -- 2020. -- Т. 37. -- Article 100286. DOI: 10.1016/j.cosrev.2020.100286.
		\item \hypertarget{rep:i26}{} Gilad G., Sharan R. From Leiden to Tel-Aviv University (TAU): exploring clustering solutions via a genetic algorithm. // PNAS Nexus. -- 2023. -- Т. 2, № 6. -- Article pgad180. DOI: 10.1093/pnasnexus/pgad180.
		\item \hypertarget{rep:i27}{} Williams C. K. I. On Suspicious Coincidences and Pointwise Mutual Information. // Neural Computation. -- 2022. -- Т. 34, № 10. -- С. 2037--2046. DOI: 10.1162/neco\_a\_01533.
		\item \hypertarget{rep:i28}{} Hassan B. A., Tayfor N. B., Hassan A. A., Ahmed A. M., Rashid T. A., Abdalla N. N. From A-to-Z review of clustering validation indices. // Neurocomputing. -- 2024. -- Т. 601. -- Article 128198. DOI: 10.1016/j.neucom.2024.128198.
		\item \hypertarget{rep:i29}{} Incitti F., Urli F., Snidaro L. Beyond word embeddings: A survey. // Information Fusion. -- 2023. -- Т. 89. -- С. 418--436. DOI: 10.1016/j.inffus.2022.08.024.
		...
		\item \hypertarget{rep:i40}{} Reddy C. K., Vinzamuri B. A survey of partitional and hierarchical clustering algorithms. // In: Data clustering. -- 2018. -- С. 87--110.
		\item \hypertarget{rep:i41}{} Fortunato S. Community detection in graphs. // Physics Reports. -- 2010. -- Т. 486, № 3--5. -- P. 75--174.
		\item \hypertarget{rep:i42}{} Traag V. A., Waltman L., van Eck N. J. From Louvain to Leiden: guaranteeing well-connected communities. // Scientific Reports. -- 2019. -- Т. 9. -- Article 5233.
		\item \hypertarget{rep:i43}{} Bouma G. Normalized (pointwise) mutual information in collocation extraction. // Proceedings of GSCL. -- 2009.
		\item \hypertarget{rep:i44}{} Rousseeuw P. J. Silhouettes: a graphical aid to the interpretation and validation of cluster analysis. // Journal of Computational and Applied Mathematics. -- 1987. -- Т. 20. -- P. 53--65.
		\item \hypertarget{rep:i45}{} Mikolov T., Chen K., Corrado G., Dean J. Efficient Estimation of Word Representations in Vector Space. // arXiv preprint arXiv:1301.3781. -- 2013.
	\end{enumerate}
	...
	PMI и его нормированные варианты (в том числе NPMI) удобны для сравнения ассоциаций разной частотности при анализе совстречаемости \srcref{27}.
	...
	Классические алгоритмы детектирования сообществ Louvain и Leiden и их практическое применение в задачах кластеризации обсуждаются в \srcmulti{25,26}.
	...
	С учётом рассмотренной постановки и требований к масштабу итоговый выбор алгоритма кластеризации в работе предлагается сделать в пользу \textbf{Leiden} как метода детектирования сообществ на взвешенных графах \srcmulti{25,26}.
	...
	\begin{enumerate}[wide=0pt, leftmargin=0pt, labelindent=\parindent, listparindent=\parindent, nosep, label=\arabic*]
		...
		\item \hypertarget{src:i24}{} Ezugwu A. E., Ikotun A. M., Oyelade O. O., Abualigah L., Agushaka J. O., Eke C. I., Akinyelu A. A. A comprehensive survey of clustering algorithms: State-of-the-art machine learning applications, taxonomy, challenges, and future research prospects. // Engineering Applications of Artificial Intelligence. -- 2022. -- Т. 110. -- Article 104743. DOI: 10.1016/j.engappai.2022.104743.
		\item \hypertarget{src:i25}{} Chunaev P. Community detection in node-attributed social networks: A survey. // Computer Science Review. -- 2020. -- Т. 37. -- Article 100286. DOI: 10.1016/j.cosrev.2020.100286.
		\item \hypertarget{src:i26}{} Gilad G., Sharan R. From Leiden to Tel-Aviv University (TAU): exploring clustering solutions via a genetic algorithm. // PNAS Nexus. -- 2023. -- Т. 2, № 6. -- Article pgad180. DOI: 10.1093/pnasnexus/pgad180.
		\item \hypertarget{src:i27}{} Williams C. K. I. On Suspicious Coincidences and Pointwise Mutual Information. // Neural Computation. -- 2022. -- Т. 34, № 10. -- С. 2037--2046. DOI: 10.1162/neco\_a\_01533.
		\item \hypertarget{src:i28}{} Hassan B. A., Tayfor N. B., Hassan A. A., Ahmed A. M., Rashid T. A., Abdalla N. N. From A-to-Z review of clustering validation indices. // Neurocomputing. -- 2024. -- Т. 601. -- Article 128198. DOI: 10.1016/j.neucom.2024.128198.
		\item \hypertarget{src:i29}{} Incitti F., Urli F., Snidaro L. Beyond word embeddings: A survey. // Information Fusion. -- 2023. -- Т. 89. -- С. 418--436. DOI: 10.1016/j.inffus.2022.08.024.
	\end{enumerate}
